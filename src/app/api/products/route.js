  ...
  try {
    ...
    const product = await prisma.products.create({
      data: {
        ...
        min_order: parseFloat(min_order)  ?? 0.0,
        store_id : parseInt(store_id),
        remarks: remarks || null,
        default_tax_account: default_tax_account || 'None',
        additional_cess: additional_cess || null,
        d_purchase_price_ex: parseFloat(d_purchase_price_ex)  ?? null,
        d_purchase_price_in: parseFloat(d_purchase_price_in) ?? null,
        d_sale_price_in: parseFloat(d_sale_price_in)  ?? null,
        d_sale_price_ex: parseFloat (d_sale_price_ex)  ?? null,
        default_discount:parseFloat(default_discount)  ?? null,
      },
      include: {
        category: { select: { category_id: true, category_name: true } },
        sub_category: { select: { sub_category_id: true, sub_category_title: true } },
